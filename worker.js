export default {
  async fetch(request, env) {
    ...
    // ---------- OCR 查询结果接口 (新增) ----------
    if (path === '/ocr/result' && request.method === 'POST') {
...
async function handleOCR(request, env) {
  try {
    ...
    // 循环提交直到成功或者重试耗尽
    while (retries < MAX_RETRIES && !jobId) {
      try {
        let resp;
        if (imageUrl) {
          const payload = {
        ...
        if (!resp.ok) {
          // 如果是队列已满(10010)，休眠几秒然后重试
          if (data.code === 10010) {
            retries++;
            await new Promise(r => setTimeout(r, 3000)); // 等待3秒
            continue; 
          }
          throw new Error('提交任务失败: ' + JSON.stringify(data));
        }

        jobId = data.data?.jobId;
        if (!jobId) throw new Error('未获取到 jobId');
        break; // 成功拿到 jobId，退出循环

      } catch (err) {
        retries++;
        if (retries >= MAX_RETRIES) throw err; // 重试耗尽则抛出错误
        await new Promise(r => setTimeout(r, 2000)); // 其他错误也等2秒
      }
    }
...
async function handleGetResult(request, env) {
  try {
    ...
    if (!jobId) {
      return new Response(JSON.stringify({ error: '缺少 jobId' }), { status: 400, headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' }});
    }
    ...
    // 如果后端已经是 done 状态，顺便把解析好的文本打包返回给前端
    if (resultData.data?.state === 'done') {
      ...
      if (jsonUrl) {
        ...
        resultData.finalText = textData.result?.layoutParsingResults?.[0]?.markdown?.text || '';
      }
    }
...
<div class="container">
  <div class="app-box" id="appBox">
    ...
    <div class="loading" id="loading"><div class="spinner"></div><p style="margin-top:8px;">识别中，请耐心等待...</p></div>
...
let pollingInterval = null; // 轮询定时器句柄
...
recognizeBtn.onclick = async () => {
  ...
  try {
    // 1. 提交任务
    const submitResp = await fetch('/ocr', {
      method: 'POST',
    ...
    if (!submitData.success || !submitData.jobId) {
      throw new Error(submitData.error || '提交任务失败，请重试');
    }

    const jobId = submitData.jobId;
    let pollCount = 0;
    const MAX_POLLS = 60; // 最多等待 60 * 2 秒 = 2 分钟
    ...
    pollingInterval = setInterval(async () => {
      ...
      if (pollCount > MAX_POLLS) {
        ...
        recognizeBtn.disabled = false;
        resultContent.textContent = '❌ 识别超时，请稍后再试';
        result.style.display = 'block';
        return;
      }

      const resultResp = await fetch('/ocr/result', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (state === 'done') {
        clearInterval(pollingInterval);
        const text = resultData.finalText || '识别完成，但未提取到文字';
        ...
        loading.style.display = 'none';
        recognizeBtn.disabled = false;
        resultContent.textContent = '❌ 识别失败: ' + JSON.stringify(resultData);
        result.style.display = 'block';
      }
      // 如果是 'running' 或者 'pending'，就循环继续等待
    }, 2000);

  } catch (e) {
    loading.style.display = 'none';
    recognizeBtn.disabled = false;
    resultContent.textContent = '❌ 错误: ' + e.message;
    result.style.display = 'block';
  }
};
